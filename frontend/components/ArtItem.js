...
function ArtItem(props) {
    ...
    if (connected) {
      if (mintedNfts) {
        if (mintedNfts.includes(props.id)) {
          return (
            <button
              className="ntf-mint nft-minted"
              onClick={() => mint(props.id)}
            >
              Minted
            </button>
          );
          ...
          return (
            <button className="ntf-mint" onClick={() => mint(props.id)}>
              Mint
            </button>
          );
        }
  ...
  return (
    <div className="category-card">
      <img src={props.image} alt="Crypto Dev NFT Gallery" className="art-img" />
      <div
        className="d-flex justify-content-between mb-0 p-0"
      ...
      <div
        ...
        style={{ height: "40px" }}
      >
        <p className="card-bd mt-1">Crypto Dev NFT #{props.id}</p>
        <p className="text-bold card-bd mt-1">
          <i className="fab fa-ethereum"></i> 0.00015
        </p>
      </div>
      {mintButton()}
      {showed ? <MintedModal description={props.id} close={close} /> : null}
    </div>
  );
}
